...
class PlayerOne extends StatefulWidget{
  @override
  _PlayerOneState createState() => new _PlayerOneState();
}
...
class _PlayerOneState extends State<PlayerOne>{
  ...
  List<GameButton> doInit(){
    activePlayer = 1;
    player1 = new List();   // listelerin burada newlenmesinin sebebi
    player2 = new List();   // resetlendiğinde listelerin tekrar sıfırlanması
  ...
  void autoPlay(){
    ...
    for(var cellId in list){
      if(!(player1.contains(cellId) || player2.contains(cellId))){    // '|| --> or'
    ...
    int i = buttonList.indexWhere((element) => element.id == cellId); //  buttonList listesindeki elemanların hepsini gez. Her elemana 'element' takma adını tak.
    playGame(buttonList[i]);                                          //  o elemanların id lerinden cellId değeri ile uyuşanı bul. Ve o elemanın gerçek index değerini döndür.
  }
  ...
  void resetGame(){
    if(Navigator.canPop(context)) Navigator.pop(context);   // böyle de çalışıyormuş.
    setState(() {
      buttonList = doInit();
    });
  }
